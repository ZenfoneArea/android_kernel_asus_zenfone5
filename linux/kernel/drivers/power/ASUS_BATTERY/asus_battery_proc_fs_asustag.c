...
int asus_battery_info_proc_write(struct file *filp, const char __user *buffer, size_t count, loff_t *ppos)
{
        return count;
}
...
int asus_battery_info_proc_read(struct file *filp, char __user *buffer, size_t count, loff_t *ppos)
{
	int len=0;
	ssize_t ret = 0;
	char *buff;
...
int asus_battery_profile_read(struct file *filp, char __user *buffer, size_t count, loff_t *ppos)
{
	int len=0;
...
int asus_battery_register_proc_fs_test(void)
{
	...
	int ent=0;
		...
		ent = proc_create("asus_battery_info", 0666,NULL, &asus_battery_info_fop); 
		if(!ent)
		{
	...
	ent = proc_create(PROCFS_BATTERY, 0666,NULL, &asus_battery_profile_fop); 
	if(!ent)
	{
